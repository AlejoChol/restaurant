...
<?php
	...
	msquli_select_db($conn,"restaurant");
	...
	while ($row = $result->fetch_assoc()){
		...
		for ($i = 0; $i < count($pedidos_detalles[$i]); $i++){
			echo '<tr>' .
				'<td>' . $pedidos_detalles[$ID_pedidoActual][$i]['Nombre'] . '</td>' .
				'<td>' . $pedidos_detalles[$ID_pedidoActual][$i]['Cant'] . '</td>' .
				'<td>' . $pedidos_detalles[$ID_pedidoActual][$i]['Precio'] . '</td>' .
			'</tr></table>';
		}
	}
?>
